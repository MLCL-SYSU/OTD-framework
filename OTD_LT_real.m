...
for e = 1 : E
    ...
    for t = 1 : T
        ...
        FoV_C_i_mod = mod(FoV_C_i, M * M_intile);
        FoV_C_i_mod(FoV_C_i_mod == 0) = M * M_intile;
        FoV_C_j = round(mean(FoV_tile_j_hat));
        FoV_C_j_mod = mod(FoV_C_j, N * N_intile);
        FoV_C_j_mod(FoV_C_j_mod == 0) = N * N_intile;
        
        phi(:, :, t) = real(acos(cos(y(FoV_C_i_mod)) * cos(y) * cos(x - x(FoV_C_j_mod)) + sin(y) * sin(y(FoV_C_i_mod))));
        Q(t) = Q_factor * FoV_tiles_num(t);
        
        %% 
        C = sqrt(2 * M * N);  

        Dq = max([abs(Q(t) - FoV_tiles_num(t) * R(1)), abs(Q(t) - FoV_tiles_num(t) * R(K))]) / (FoV_tiles_num(t));
        Db = R(K) - R(1);
        D = max([Dq, Db]);
        
        Gf = (sqrt((1 + alpha^2) * sum(R.^2)) + 2 * beta * R(K)^2) / 3;
        Gq = sqrt(FoV_tiles_num(t) * sum(R.^2)) / sqrt(FoV_tiles_num(t));
        ...
        QoE1(t, e) = (W1_vector(:, t) .* R_vector)' * X_vector(:, t);
        ...
        QoE2(t, e) = -(W2_vector(:, t) .* R_vector)' * X_vector(:, t);
        
        W3_temp = squeeze(sum(sum(reshape(FoV(:, :, t), M_intile, M, N_intile, N), 1), 3));
        W3_temp(W3_temp ~= 0) = 1;
        W3(:, :, t) = W3_temp;
        W3_vector(:, t) = matrix2extended_vector(W3(:, :, t), M, N, K);  
        QoE3(t, e) = -((W3_vector(:, t) .* R_vector)'.^2  * X_vector(:, t).^2) / (FoV_tiles_num(t));
        
        f(t) = -(QoE1(t, e) + alpha * QoE2(t, e) + beta * QoE3(t, e)) / 3;
        ...
        DX_g_q = (-W3_vector(:, t) .* R_vector) / sqrt(FoV_tiles_num(t));
        DX_g_b = (R_vector) / sqrt(M * N);
        DX_Lagrangian = DX_f + lambda_q(t) * DX_g_q + lambda_b(t) * DX_g_b + DX_h * mu(:,t);       
        ...
        Dlambda_q_Lagrangian = g_q(t) - delta * eta * lambda_q(t);
        lambda_q(t + 1) = max(lambda_q(t) + eta * Dlambda_q_Lagrangian, 0);

        Dlambda_b_Lagrangian = g_b(t) - delta * eta * lambda_b(t);
...
%% 
file_name_format = 'OTD_LT_real_%s_T(%d)_E(%d)_noise(mu_%d)_B(mu_%d_sigma_%d)_Q(%d)_';
