...
Dq = max([abs(qr - m * n * R(1)), abs(qr - m * n * R(K))]) / (m * n);
Db = R(K) - R(1);
D = max([Dq, Db]);
...
tda_options_opti = optimoptions("fmincon","MaxFunctionEvaluations",18000,"OptimalityTolerance",1.00e-4);
...
for e = 1 : E
    ...
    for t = 1 : T
        ...
        g_q(t) = (qr - (W3_vector(:, t) .* R_vector)' * X_vector(:, t)) / sqrt(m * n);
        ...
        DX_g_q = (-W3_vector(:, t) .* R_vector) / sqrt(m * n); % 
...
%%
file_name_format = 'OTD_LT_synthetic_T(%d)_E(%d)_noise(mu_%d)_B(mu_%d_sigma_%d)_Q(%d)_';
file_name = [sprintf(file_name_format, T, E, noise_mu, 100 * B_mu / (M * N), 10 * B_sigma_factor, qr * 40 / (m * n)), date];
save(file_name);
